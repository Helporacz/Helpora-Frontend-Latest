...
const KYCStatus = () => {
  ...
  const statusBadge = (status) => {
    ...
    return <Badge bg={map[status] || "secondary"} className="" style={{marginBottom:"0px", fontSize:"0.85rem"}}>{status?.toUpperCase()}</Badge>;
  };
  ...
  const rowData = filtered.map((item) => ({
    data: [
      ...
      { value: item.submittedAt ? new Date(item.submittedAt).toLocaleDateString() : "-" },
      { value: statusBadge(item.status) },
      {
        value: (
          <button
            onClick={() => navigate(`/approve-kyc/${item._id}`)}
            className="px-4 py-1 fs-6"
            style={{border:"none", backgroundColor:"#6fadfaff", color:"white", borderRadius:"15px"}}
          >
            View
          </button>
        ),
      ...
      <div className="services-header-block">
        <div className="content-block">
          <div className="title-value-block d-flex align-items-center gap-2 mb-3">
            <div className="text-20-700 color-black-100">{t("approvedKYC.header")}</div>
          </div>
